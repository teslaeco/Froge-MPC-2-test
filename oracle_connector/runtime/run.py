...
def make_material(name, rgb, pattern='plain', roughness=0.7, metallic=0.0):
    ...
    return _create_material(name, rgb, pattern, roughness, metallic)


def _create_material(name, rgb, pattern='plain', roughness=0.7, metallic=0.0):
...
def edit_material_factory():
    """A separate bounded palette for edits after the trusted anatomy build.

    A valid eight-material plan can already produce sixteen used materials:
    eyes, nails and skin variants are generated by the trusted renderer. Count
    newly authored edit materials instead of rejecting that existing palette.
    Only actual surviving helper-created datablocks earn export allowance.
    """
    budget = {'created': 0, 'materials': set(), 'images': set()}

    def create(name, rgb, pattern='plain', roughness=0.7, metallic=0.0):
        if budget['created'] >= 8:
            raise ValueError('Edit limit: at most 8 new materials across all edits. '
                             'Reuse an existing material with bpy.data.materials.get(name).')
        images_before = {item.as_pointer() for item in bpy.data.images}
        material = _create_material(name, rgb, pattern, roughness, metallic)
        budget['created'] += 1
        budget['materials'].add(material.as_pointer())
        budget['images'].update(item.as_pointer() for item in bpy.data.images
                                if item.as_pointer() not in images_before)
        return material
...
def ellipsoid(name, center, scale=None, material=None, subdivisions=4, *, radii=None):
    # Older MCP contracts advertised radii, while the implementation used
    # scale. Support both spellings and preserve all positional callers.
    if scale is not None and radii is not None:
        raise ValueError('ellipsoid accepts scale or radii, not both.')
    scale = radii if scale is None else scale
    if scale is None:
        raise ValueError('ellipsoid needs scale or radii.')
...
def finish(output=None, *, edit_budget=None, anatomy_before=None):
    ...
    edit_budget = edit_budget or {'created': 0, 'materials': set(), 'images': set()}
    edit_materials = sum(item.as_pointer() in edit_budget['materials'] for item in bpy.data.materials)
    edit_images = sum(item.as_pointer() in edit_budget['images'] for item in bpy.data.images)
    material_limit = 16+4*extra+projection_materials+edit_materials
    image_limit = 16+4*extra+projection_images+edit_images
    if edit_budget['created'] > 8 or edit_materials > 8 or edit_images > 16:
        raise ValueError('Edit palette exceeds 8 new materials or 16 helper texture images.')
    if len(bpy.data.materials) > material_limit or len(bpy.data.images) > image_limit:
        raise ValueError('Export limit: %d used materials and %d used texture images. '
                         'Plans allow 8 shared materials and edits 8 additional materials. '
                         'Actual: %d materials, %d images.' %
                         (material_limit,image_limit,len(bpy.data.materials),len(bpy.data.images)))
...
def execute_job(folder=Path('/work')):
    ...
    if (folder/'image3d-manifest.json').is_file():
        ...
        raise SystemExit(0)
    has_scene = (folder/'scene.json').is_file()
    if has_scene:
