"""Deterministic v35 worker update with export-bound acceptance."""
import base64
import gzip
import hashlib
import io
...
template = (root / 'scripts/cloud-shell-v35.template.py').read_text()
...
compile(program, 'froge-v35.py', 'exec')
readme = """FORGE v35 — Astra + Codex + Blender MCP

Cloud Shell: Menu > Upload > froge-v35.zip
...
Wymagane: CODEX_MCP_BLENDER_BUILD_OK i FROGE_V35_OK.

V35 naprawia kontrole atlasu glowy w grupach wezlow i materialach obiektu.
...
W /v1/health wymagane: connectorVersion=35,
referenceAcceptanceRevision=1 i workerRelease=v35-reference-acceptance.
...
python3 "$HOME/froge-v35/froge-v35.py" --test-job NUMER_ZAPISANEGO_ZLECENIA
...
(folder / 'froge-v35.py').write_text(program)
buffer = io.BytesIO()
with zipfile.ZipFile(buffer, 'w', zipfile.ZIP_DEFLATED) as archive:
    for name, value in [('froge-v35.py', program), ('CZYTAJ.txt', readme),
                        ('POLECENIE-CODEX.txt', (root/'docs/reviews/v35/POLECENIE-CODEX.txt').read_text())]:
...
try:
    with zipfile.ZipFile(temporary) as verified:
        if verified.testzip() is not None or verified.read('froge-v35.py') != program.encode():
            raise RuntimeError('Incomplete v35 update archive')
    temporary.replace(folder / 'froge-v35.zip')
finally:
    temporary.unlink(missing_ok=True)
print('FORGE v35:', (folder / 'froge-v35.zip').stat().st_size, 'bytes;', len(names), 'files; payload', hashlib.sha256(raw).hexdigest())
