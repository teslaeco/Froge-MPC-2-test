#!/usr/bin/env python3
"""FORGE v28: Codex with Blender MCP, reference texture protection and actual execution."""
import base64
import gzip
import hashlib
import json
...
def verify_export(target):
    ...
    with tempfile.TemporaryDirectory(prefix='v28-projection-install-check-', dir=target/'state') as temporary:
        ...
        print('FROGE_V28_GEOMETRY_PROJECTION_OK: narzedzia i eksporty sprawdzone.',flush=True)
...
def install_on_oracle(data):
    ...
    print('FROGE_V28_OK: Astra + Blender zainstalowane. Test eksportow zakonczony; nie generowano postaci przez API.', flush=True)
...
if __name__ == '__main__':
    try:
        ...
        if isinstance(error, subprocess.CalledProcessError):
            print('FROGE_V28_ERROR: operacja nie powiodla sie. Przyczyna jest powyzej.', file=sys.stderr)
        elif isinstance(error, subprocess.TimeoutExpired):
            print('FROGE_V28_ERROR: limit czasu polaczenia. Sprawdz status Oracle przed ponowieniem.', file=sys.stderr)
        else:
            print('FROGE_V28_ERROR: ' + str(error), file=sys.stderr)
        raise SystemExit(1)
