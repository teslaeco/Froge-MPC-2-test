#!/usr/bin/env python3
"""FORGE v27: Codex with Blender MCP, reference texture protection and actual execution."""
import base64
import gzip
import hashlib
import json
...
def verify_export(target):
    ...
    with tempfile.TemporaryDirectory(prefix='v27-projection-install-check-', dir=target/'state') as temporary:
        ...
        print('FROGE_V27_GEOMETRY_PROJECTION_OK: narzedzia i eksporty sprawdzone.',flush=True)
...
def install_on_oracle(data):
    ...
    if db_path.is_file():
        with sqlite3.connect('file:'+str(db_path)+'?mode=ro',uri=True) as db:
            for jid,state in db.execute('SELECT id,state FROM jobs ORDER BY updated DESC LIMIT 3'):
                ...
                print('FROGE_V27_DIAGNOSTIC:',jid,state,'plan=',(folder/'scene.json').is_file(),
                ...
                for diagnostic in ('attempt-1-error.json','attempt-2-error.json','failure.json','timing.json'):
                    ...
                    if path.is_file() and not path.is_symlink() and path.stat().st_size<16000:
                        try:
                            ...
                            print('FROGE_V27_DIAGNOSTIC',diagnostic,json.dumps(fields,ensure_ascii=True)[:1800],flush=True)
                        except (OSError,ValueError):pass
    with tempfile.TemporaryDirectory(prefix='froge-v27-update-') as temporary:
    ...
    print('FROGE_V27_OK: Astra + Blender zainstalowane. Test eksportow zakonczony; nie generowano postaci przez API.', flush=True)
...
if __name__ == '__main__':
    try:
        ...
        if isinstance(error, subprocess.CalledProcessError):
            print('FROGE_V27_ERROR: operacja nie powiodla sie. Przyczyna jest powyzej.', file=sys.stderr)
        elif isinstance(error, subprocess.TimeoutExpired):
            print('FROGE_V27_ERROR: limit czasu polaczenia. Sprawdz status Oracle przed ponowieniem.', file=sys.stderr)
        else:
            print('FROGE_V27_ERROR: ' + str(error), file=sys.stderr)
        raise SystemExit(1)
