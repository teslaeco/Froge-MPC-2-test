#!/usr/bin/env python3
"""FORGE v24: bounded timeouts, terminal SSE and recoverable model checkpoints."""
import base64
import gzip
import hashlib
import json
...
def verify_export(target):
    ...
    with tempfile.TemporaryDirectory(prefix='v24-projection-install-check-', dir=target/'state') as temporary:
        ...
        print('FROGE_V24_GEOMETRY_PROJECTION_OK: narzedzia i eksporty sprawdzone.',flush=True)
...
def install_on_oracle(data):
    ...
    if db_path.is_file():
        with sqlite3.connect('file:'+str(db_path)+'?mode=ro',uri=True) as db:
            for jid,state in db.execute('SELECT id,state FROM jobs ORDER BY updated DESC LIMIT 3'):
                ...
                print('FROGE_V24_DIAGNOSTIC:',jid,state,'plan=',(folder/'scene.json').is_file(),
                      'GLB=',(folder/'model.glb').is_file(),flush=True)
    with tempfile.TemporaryDirectory(prefix='froge-v24-update-') as temporary:
    ...
    print('FROGE_V24_OK: Astra + Blender zainstalowane. Test eksportow zakonczony; nie generowano postaci przez API.', flush=True)
...
if __name__ == '__main__':
    try:
        ...
        if isinstance(error, subprocess.CalledProcessError):
            print('FROGE_V24_ERROR: operacja nie powiodla sie. Przyczyna jest powyzej.', file=sys.stderr)
        elif isinstance(error, subprocess.TimeoutExpired):
            print('FROGE_V24_ERROR: limit czasu polaczenia. Sprawdz status Oracle przed ponowieniem.', file=sys.stderr)
        else:
            print('FROGE_V24_ERROR: ' + str(error), file=sys.stderr)
        raise SystemExit(1)
