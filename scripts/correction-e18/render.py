...
P=Path('/workspace/scratch/584c9d97a5a1/correction-e18');ROOT=P.parent
mode=sys.argv[sys.argv.index('--')+1] if '--' in sys.argv else 'after'
asset=ROOT/'upload/FORGE-model-E15.fbx' if mode=='before' else P/'FORGE-model-E18.fbx'
...
# Extra high-resolution upper-body view from the same imported final FBX.
if mode=='after':
 scene.view_layers[0].material_override=None;scene.render.resolution_x=2048;scene.render.resolution_y=2560;scene.cycles.samples=32
 target=Vector((0,.05,1.385));camera.location=target+Vector((0,-3,0));camera.rotation_euler=(target-camera.location).to_track_quat('-Z','Y').to_euler();camera.data.ortho_scale=.86;scene.render.filepath=str(P/'E18-HQ-2048x2560.png');bpy.ops.render.render(write_still=True);print('HQ_FINAL_FBX_RENDERED',flush=True)
