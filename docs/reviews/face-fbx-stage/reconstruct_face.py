...
for ob in selected:
 ...
 layer=ob.data.uv_layers.active or ob.data.uv_layers.new(name='UVMap');ob.data.uv_layers.active_index=0;ob.data.uv_layers[0].active_render=True
...
enamel=material('Natural ivory enamel',(.64,.58,.44),.36);dark=material('Mouth interior',(.034,.009,.006),.92)
...
for side in (-1,1):
 ...
 for j,w in enumerate((.0087,.0074,.0068,.0062,.0060,.0056)):
  ...
  tooth('Anatomical_upper_tooth_%s_%s'%(side,j),x,y,z,w,.010 if j<2 else .011 if j==2 else .008,.008,side*.7*(abs(x)/.06))
  if side>0:tooth('Anatomical_lower_tooth_%s'%j,x,y+.0005,z-.012,w*.92,.0075,.007,side*.7*(abs(x)/.06))
...
bpy.context.scene['reference_likeness_accepted']=False;bpy.context.scene['repair_revision']='r7-reference-fit'
bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'FORGE-twarz-r7.blend'))
bpy.ops.export_scene.gltf(filepath=str(OUT/'FORGE-twarz-r7.glb'),export_format='GLB',export_extras=True,export_cameras=False,export_lights=False)
report={'revision':'r7','hair_sha256_before':hair_before,'hair_sha256_after':hair_identity(),'hair_preserved':True,'reference_landmarks':len(matches),'removed_mouth_faces':len(remove),'mapped_front_faces':mapped,'reference_lighting_removed':False,'unseen_depth':'inferred','likeness_accepted':False,'landmarks':matches}
(OUT/'face-fit-report.json').write_text(json.dumps(report,indent=2));print('R7_FACE_SAVED',flush=True)
