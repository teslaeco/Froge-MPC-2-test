...
# A continuous sampled hairline replaces selection of coarse face polygons.
from rooted_hair import scalp_tree
modifier=head.modifiers.new('Smooth anatomy outlines','SUBSURF');modifier.levels=1
bpy.context.view_layer.update();tree=scalp_tree(head)
vertices=[];faces=[];rows=26;cols=160;centre=Vector((0,.065,1.59))
for i in range(rows+1):
 f=max(.006,i/rows)
 for j in range(cols):
  theta=math.tau*j/cols
  phi=f*(.92+.92*(1-math.cos(theta))*.5+.15*abs(math.sin(theta)))
  direction=Vector((math.sin(phi)*math.sin(theta),-math.sin(phi)*math.cos(theta),math.cos(phi)))
  hit,n,_,_=tree.ray_cast(centre+direction*.6,-direction)
  if hit is None:hit,n,_,_=tree.find_nearest(centre+direction*.14)
  vertices.append(tuple(hit+n*.0025))
  if i:faces.append(((i-1)*cols+j,(i-1)*cols+(j+1)%cols,i*cols+(j+1)%cols,i*cols+j))
mesh=bpy.data.meshes.new('Smooth fitted scalp');mesh.from_pydata(vertices,[],faces);mesh.update()
cap=bpy.data.objects.new('Reference_fitted_scalp',mesh);bpy.context.collection.objects.link(cap);mesh.materials.append(mat)
uv=mesh.uv_layers.new(name='HairUV')
for poly in mesh.polygons:
 poly.use_smooth=True
 for li in poly.loop_indices:
  vi=mesh.loops[li].vertex_index;uv.data[li].uv=(vi%cols/cols,vi//cols/rows)
...
for j in range(64):
 ...
 hair_lock('Reference_back_lock_%02d'%j,head,points,mats[j%3],width=rng.uniform(.009,.014),depth=rng.uniform(.010,.016),wave=rng.uniform(.017,.029),turns=rng.uniform(2.0,2.8),seed=440+j,steps=90)
...
for side in (-1,1):
 for j in range(20):
  ...
  hair_lock('Reference_front_lock_%s_%02d'%(side,j),head,points,mats[j%3],width=rng.uniform(.009,.014),depth=rng.uniform(.010,.017),wave=rng.uniform(.027,.035),turns=rng.uniform(2.0,2.6),seed=800+j+(100 if side>0 else 0),steps=100)
...
bpy.context.scene['repair_revision']='direct-model-r5'
bpy.context.scene['reference_likeness_accepted']=False
for im in bpy.data.images:
 if im.has_data and not im.packed_file:im.pack()
bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'FORGE-model-poprawka-r5.blend'))
bpy.ops.export_scene.gltf(filepath=str(OUT/'FORGE-model-poprawka-r5.glb'),export_format='GLB',export_extras=True,export_cameras=False,export_lights=False)
print('REPAIR_R5_SAVED',flush=True)
