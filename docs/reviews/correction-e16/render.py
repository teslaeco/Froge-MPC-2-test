...
P=Path('/workspace/scratch/584c9d97a5a1/correction-e16');ROOT=P.parent
mode=sys.argv[sys.argv.index('--')+1] if '--' in sys.argv else 'after'
asset=ROOT/'upload/FORGE-model-E15.fbx' if mode=='before' else P/'FORGE-model-E16.fbx'
...
scene.render.threads_mode='FIXED';scene.render.threads=8;scene.render.resolution_x=640;scene.render.resolution_y=800;scene.render.resolution_percentage=100
