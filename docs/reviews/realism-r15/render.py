...
views=[('face',face,scale,0),('front',Vector((0,.02,1.15)),1.32,0),('face-angle',face,scale,.55),('left',Vector((0,.02,1.15)),1.32,math.pi/2),('back',Vector((0,.02,1.15)),1.32,math.pi)]
...
# Neutral material confirms crown geometry and eye position independent of photo colour.
clay=bpy.data.materials.new('R14 geometry review');clay.use_nodes=True;clay.node_tree.nodes.get('Principled BSDF').inputs['Base Color'].default_value=(.38,.38,.38,1);clay.node_tree.nodes.get('Principled BSDF').inputs['Roughness'].default_value=.8
...
report={'fbx_sha256':hashlib.sha256(asset.read_bytes()).hexdigest(),'fbx_reimport_verified':True,'triangles':triangles,'mesh_objects':len(meshes),'bounds':[list(low),list(high)],'missing_textures':missing,'meshes_with_uv':sum(bool(o.data.uv_layers) for o in meshes),'renders':'actual reimported FBX','wave_guides_retained':source['wave_guides_retained'],'likeness_accepted':False,'print_ready':False,'anatomical_eyes':2,'fine_hair_fibres':source['long_fibres']+source['root_fibres']}
