...
async function submit() {
  vi.stubGlobal('fetch', vi.fn(async url => Response.json(String(url).endsWith('/health') ? { connectorVersion: 7 } : { state: 'queued' }, { status: 202 })))
...
describe('private Blender request lifecycle with real SQLite', () => {
  it.each([5, 6])('rejects generation on worker v%i before queuing', async version => {
    ...
    expect((await response.json()).error).toContain('geometry-v7')
