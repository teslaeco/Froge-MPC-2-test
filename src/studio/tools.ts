...
export const studioTools: WebMcpToolDefinition[] = [
  { ...common, name: 'inspect_live_3d_studio', readOnly: true, description: 'Read the editable model specification in the local parametric tab in millimetres, revision and exact mesh bounds. Supported shapes: rocket, cylinder, cone, sphere. No external AI is connected.', inputSchema: { type: 'object', properties: {}, additionalProperties: false }, execute: async () => {
    const s = getStudio(); return { state: 'PASS', spec: s.spec, revision: s.revision, qa: createModel(s.spec).qa, llmConnected: false }
  } },
  { ...common, name: 'update_live_3d_studio', readOnly: false, description: 'Update the model in the local parametric tab (not the generated AI model). Supply a COMPLETE specification, millimetres, and the expectedRevision read by inspect_live_3d_studio. Refuses stale edits. No arbitrary mesh generation or production.', inputSchema: { type: 'object', properties: { spec: z.toJSONSchema(specSchema), expectedRevision: { type: 'integer', minimum: 1 } }, required: ['spec', 'expectedRevision'], additionalProperties: false }, execute: async (input: unknown) => {
