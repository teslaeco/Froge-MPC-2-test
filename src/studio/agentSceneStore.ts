...
export function requestAgentModel(prompt:string){if(!prompt.trim()||prompt.length>5000)throw new Error('Wpisz opis od 1 do 5000 znaków.');state={...state,revision:state.revision+1,request:{id:crypto.randomUUID(),prompt:prompt.trim()},status:'waiting',note:'Polecenie przygotowane. Poproś agenta przeglądarki o wykonanie albo skopiuj je do rozmowy z Codexem.'};emit();return state.request}
