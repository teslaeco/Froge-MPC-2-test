...
import { AmbientLight, Box3, Color, DirectionalLight, PerspectiveCamera, Scene, Vector3, WebGLRenderer, type Group } from 'three'
import { OrbitControls } from 'three/addons/controls/OrbitControls.js'

export default function AiModelViewer({ model }: { model: Group }) {
  const host = useRef<HTMLDivElement>(null), controls = useRef<OrbitControls | null>(null)
  ...
  useEffect(() => {
    ...
    const fill = new DirectionalLight(0xb4d8ff, 2); fill.position.set(-3,1,-3); scene.add(fill)
    ...
    const resize = () => { const width = element.clientWidth, height = element.clientHeight; if (!width || !height) return; renderer.setSize(width, height); camera.aspect = width / height; camera.updateProjectionMatrix(); camera.position.copy(center).add(new Vector3(1,.6,1.7).normalize().multiplyScalar(max * 2 / Math.min(camera.aspect, 1))); orbit.update() }
    resize(); orbit.saveState(); const observer = new ResizeObserver(resize); observer.observe(element)
    renderer.setAnimationLoop(() => { orbit.update(); renderer.render(scene, camera) })
    return () => { observer.disconnect(); renderer.setAnimationLoop(null); orbit.dispose(); renderer.dispose(); renderer.domElement.remove(); controls.current = null; scene.remove(model) }
  }, [model])
  return <div className="ai-viewer-wrap"><div ref={host} className="ai-viewer" />{error && <p role="alert" className="studio-error">{error}</p>}<div className="ai-camera"><button onClick={() => controls.current?.reset()}>Przywróć widok</button><span>Obróć palcem · przybliż dwoma</span></div></div>
}
