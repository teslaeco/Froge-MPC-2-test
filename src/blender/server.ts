...
export async function blenderApi(request: Request, env: BlenderEnv): Promise<Response> {
  ...
  try {
    ...
    if (url.pathname === '/api/blender/jobs') {
      ...
      if ((capabilities.connectorVersion || 1) < 7) throw new ApiError('Zainstaluj aktualizację froge-oracle-geometry-v7.zip na Oracle. Nowe modele wymagają poprawionej geometrii.', 409)
