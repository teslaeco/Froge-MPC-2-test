...
export async function blenderApi(request: Request, env: BlenderEnv): Promise<Response> {
  ...
  try {
    ...
    if (url.pathname === '/api/blender/jobs') {
      ...
      if ((capabilities.connectorVersion || 1) < 8) throw new ApiError('Zainstaluj aktualizację froge-oracle-characters-v8.zip na Oracle. Nowe modele wymagają poprawionej geometrii.', 409)
