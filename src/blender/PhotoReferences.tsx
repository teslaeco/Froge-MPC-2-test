...
export function PhotoReferences({ photos, onChange, disabled, onPreparing }: { photos: PhotoInput[]; onChange: (photos: PhotoInput[]) => void; disabled: boolean; onPreparing: (busy: boolean) => void }) {
  ...
  return <section className="photo-references" aria-label="Zdjęcia do modelu 3D">
    ...
    <label>Jakość referencji i materiałów<select value={textureMaxSize} disabled={disabled || preparing} onChange={e => setTextureMaxSize(Number(e.target.value) as TextureMaxSize)}><option value={2048}>Do 2K · szybki podgląd</option><option value={4096}>Do 4K · szczegóły</option><option value={8192}>Do 8K · materiały maksymalne</option></select></label>
    ...
    <p className="studio-helper">Do 12 MB na plik źródłowy, 2 MB po przygotowaniu i 6 MB na zlecenie. Zachowujemy proporcje i piksele do wybranego limitu. Nie powiększamy małych zdjęć do 4K/8K. Detale materiałów liczymy od nowa do 4K; ostrość fotografii zależy od źródła. Tryb 8K wymaga 10 GiB wolnej pamięci na serwerze. Zmiana limitu dotyczy kolejnych dodawanych zdjęć. Zapis następuje po kliknięciu „Generuj”. Wynik jest przybliżonym modelem, a niewidoczne powierzchnie są szacowane.</p>
